...
### Multi-Image Mode（多图拼贴模式 v1.5 新增）

在 Collage Poster 顶部提供 `[ 单图索引 SINGLE ]` 与 `[ 多图拼贴 MULTI ]` 无缝模式切换：

1. **单图与多图共存**：默认保持经典的单图索引工作流；切换为“多图拼贴”时展开左侧 **Image Tray（图像素材栏）**，支持批量上传 1 张主图 + 3~6 张辅图素材。
2. **素材栏管理**：素材栏展示缩略图、文件名与 `主图 MAIN` 标识，提供“设为主图”、“+ 添加卡片 (计数)”与“删除素材”等快速操作。
3. **辅图卡片系统**：辅图卡片独立存在于画布上，支持自由拖拽、右下角手柄等比缩放、旋转；在 Inspector 中拥有专属配置（边框颜色、线宽、衬底颜色、独立滤镜系统：黑白/亮度/对比度/饱和度、半调网点、扫描线、颗粒、纸张、脏版、轮廓描边等），且支持“按住查看原图 (HOLD FOR BEFORE)”。
4. **跨图索引与局部放大**：
   - 索引框（Index Frame）支持绑定主图或任意一张辅图卡片（可在属性面板“图源绑定”下拉框中动态切换，或选中辅图卡片后一键点击“在此卡片上添加索引框”）；
   - 裁切几何计算自适应辅图卡片的位置与旋转角度，确保跨图局部放大（Detail Crop）准确采撷对应卡片的高清像素；
   - 连接线（Connector）清晰串联辅图上的索引框与放大特写。
5. **构图与导出兼容**：辅图卡片完整参与图层前后排序、REMIX 构图保留、快捷键复制删除、Undo/Redo 历史记录，并在最终 900 × 1200 PNG 导出中完整高保真呈现。
